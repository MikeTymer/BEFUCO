...
class intro (generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['Companyentries'] = CompanyDetails.objects.filter(CompanyDetails=self.object)
        return context
...
class videos (generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['company_entries'] = self.pagination(self.request)
        return context

    def pagination(self, request):
        all_company_entries = CompanyDetails.objects.all()
        paginator = Paginator(all_company_entries, self.paginate_by2)
        page = request.GET.get('page')
        company_entries = paginator.get_page(page)
...
class category(generic.ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        all_company_entries = CompanyDetails.objects.all()
        company_entries = all_company_entries
...
class contactUs(SuccessMessageMixin, generic.CreateView):
    ...
    success_url = "/"
    success_message = "Your query has been submited , We will contact you soon."
    ...
    def get_context_data(self, **kwargs):
        ...
        context['company_entries'] = CompanyDetails.objects.all()
